...
epochs = 5000
learning_rate = 0.01  # 0.0001 is good
...
for epoch in range(epochs):
    ...
    if is_shuffle:
        ...
        y_shuffle = y_shuffle[shuffle_idx]
    ...
    for b_idx in range(batch_num):
        ...
        loss = (y_shuffle[s:e]-y_hat).pow(2).mean()       # loss = (y-y_hat)**2
...
# --- plot loss surface --- #
n = 200
weights = np.linspace(min(weights), max(weights), n)
biases = np.linspace(min(biases), max(biases), n)
...
plot.contour_2d(weights, biases, losses)
plot.contour_3d(weights, biases, losses)
