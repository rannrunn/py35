...
class Form(QWidget):
	def __init__(self):
		...
		layout_left = QBoxLayout(QBoxLayout.LeftToRight)
		layout_left.addWidget(QTextEdit())
		layout_base.addLayout(layout_left)

		layout_right = QBoxLayout(QBoxLayout.TopToBottom, self)
		layout_base.addLayout(layout_right)
		index = 0
		while(index < 8):
			self.addLayoutLoadMargin(layout_right)
			index += 1

		for i in range(layout_right.count()):
			myWidget = layout_right.itemAt(i)
			myWidget.itemAt(0)
			myWidget.itemAt(1)
			myWidget.itemAt(2)
			print(myWidget)

	def addLayoutLoadMargin(self, layout_base):
		layout_load = QBoxLayout(QBoxLayout.LeftToRight, self)
		layout_load.addWidget(QPushButton("Butoon " + str(layout_base.count() + 1)))
		layout_load.addWidget(QLineEdit())
		layout_load.addWidget(QLineEdit())
		layout_base.addLayout(layout_load)
