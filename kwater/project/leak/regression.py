...
def main(conn):
    ...
    try:
        ...
        print('water regression : ckpt name', ckpt_name)
        ...
        with tf.Session() as sess:
            print('water regression : session run')
            ...
            if (option == 'start'):
                ...
                print('water regression : Model restored.')
            ...
            try:
                if (option != 'predict'):
                    for step in range(epochs):
                        ...
                        if step % 200 == 0:
                            ...
                            conn.send(('water regression : step : ' + str(step)).encode())
                            conn.send(('water regression : hyperparameter : ' + str(step) + str(step_loss) + str(sess.run(W1)) + str(sess.run(W2)) + str(sess.run(b))).encode())
                        if step != 0 and step % 5000 == 0:
                            # Save the variables to disk.
                            save_path = saver.save(sess, ckpt_dir)
                            print('water regression : Model saved in file: %s' % save_path)
                    # Save the variables to disk.
                    save_path = saver.save(sess, ckpt_dir)
                    print('water regression : Model saved in file: %s' % save_path)
                ...
                # 조건부 리스트
                print('water regression : 차이가 ', difference, ' 이상인 데이터')
                ...
                print('water regression : Predict MSE Cost:', predict_cost_mse)
                print('water regression : Predict ABS Cost:', predict_cost_abs)


                print('water regression : ckpt name : ', ckpt_name)
                print('water regression : datetime : ', datetime.datetime.now())
                print('water regression : epochs : ', epochs)
                print('water regression : Total time : %f' % (time.time() - start))

                sess.close()
            except Exception as ex:
                print('water regression : Exception sess run : ', ex)
                pass
            finally:
                sess.close()
