...
def print_param(param):
    for key in vars(param):
        print(key + ":", round(getattr(param, key).data[0], 2))
    # params = []
...
class Param:
    ...
    def __init__(self):
        ...
        self.weight_for_final_output = None
        self.bias_for_final_output = None
    ...
    def initialize_weights(self, min_=-0.1, max_=0.1):
        ...
        self.weight_for_final_output = rand_arr(size, min_=min_, max_=max_)
        self.bias_for_final_output = rand_arr(size, min_=min_, max_=max_)
    ...
    def change_weight_to_var(self):
        ...
        self.weight_for_final_output = numpy_to_var(self.weight_for_final_output, requires_grad=True).type(
            torch.DoubleTensor)
        self.bias_for_final_output = numpy_to_var(self.bias_for_final_output, requires_grad=True).type(torch.DoubleTensor)
...
def forward_calc(x, output_before, cell_before_forget_gate, param):
    ...
    output_final = param.weight_for_final_output * output + param.bias_for_final_output

    return cell, output, output_final
...
#             cell = data[0]
#
#         cell, output, output_final = forward_calc(x=data[idx], output_before=output, cell_before_forget_gate=cell, param=param)
#         outputs.append(output)
#         output_finals.append(output_final)
#
#         if verbose is True:
#             print("Cell:  ", cell, "\nOutput:", output, "\n")
#
#     return output, outputs, output_finals
...
def lstm_forward_propagation_with_missing_data(data, param, verbose=False):
    ...
    for idx in range(length):
        ...
        if isnan(data[idx]):
            # cell, output, output_final = forward_calc(x=output, output_before=output,
            #                                           cell_before_forget_gate=cell, param=param)
            cell, output, output_final = forward_calc(x=output_final, output_before=output,
                                                      cell_before_forget_gate=cell, param=param)
        else:
            cell, output, output_final = forward_calc(x=data[idx], output_before=output,
                                                      cell_before_forget_gate=cell, param=param)
        outputs.append(output)
        output_finals.append(output_final)
