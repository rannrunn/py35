...
def plot_pqms_decomposition(period):
    ...
    dir_output = 'C:\\_data\\pqms_load_decompose_plot\\' + str(period) + '\\'
    ...
    for root, dirs, files in os.walk(dir_source):
        for file in files:
            if file.endswith(".xls") and file.find('3상') > -1:
                ...
                if os.path.join(root, file) == 'C:\_data\부하데이터\수원경기본부\금촌변전소\ID5\금촌_3상.xls':
                    bool_filename = True
                ...
                if bool_possible:
                    ...
                    df_temp = decompose(df_temp['load'], period=period)
...
if __name__ == '__main__':
    max_period = 1
    list_pool_input = [i for i in range(1, max_period + 1, 7)]
    with Pool(processes=8) as pool:
        pool.map(plot_pqms_decomposition, list_pool_input)
        pool.close()
        pool.join()
