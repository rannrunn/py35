...
def get_data(batch_size, predict, samples, data):
    ...
    for i in range(batch_size):
        t = np.arange(0, samples + predict) / Fs
        ran = random.choice(range(1000 - predict))
        y = data[ran : ran + samples + predict]
...
if __name__ == '__main__':
    ...
    print(df['current_5576'].values)

    t, y, t_next, y_next = get_data(1, 42, 42, list(df['current_5576'].values))
