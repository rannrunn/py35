...
limit_data = {'TEMP':[-15, 65], 'HUMI':[0, 100], 'PITCH':[-180, 180], 'ROLL':[-90, 90], 'AMBIENT':[0, 81900], 'UV':[0, 20.48], 'PRESS':[0, 1100], 'BATTERY':[0, 100], 'SHOCK':[0, 1], 'USN':[0, 3000], 'NTC':[-20, 120], 'UVC':[0, 10]}
limit_ylim = {'TEMP':[-15, 65], 'HUMI':[-5, 105], 'PITCH':[-185, 185], 'ROLL':[-95, 95], 'AMBIENT':[-100, 5000], 'UV':[-2, 5], 'PRESS':[-10, 1110], 'BATTERY':[-5, 105], 'SHOCK':[-1, 2], 'USN':[-10, 290], 'NTC':[-25, 125], 'UVC':[-1, 8]}
...
# SELECT
def selectAmbient(cur, variable):
    query = """SELECT POLE_ID, SENSOR_ID, (SELECT PART_NAME FROM TB_IOT_POLE_INFO_DUMMY WHERE SENSOR_ID = S1.SENSOR_ID) AS PART_NAME
                , CONCAT(LEFT(TIME_ID, 10), 'T00:00:00') AS TIME_START
                , CONCAT(LEFT(TIME_ID, 10), 'T23:59:59') AS TIME_END
                FROM TB_IOT_POLE_SECOND S1
                WHERE %s IS NOT NULL
                AND CONVERT(%s, DECIMAL(8,3)) > 77
                AND CONVERT(%s, DECIMAL(8,3)) != 60000
                AND POLE_ID != ''
                AND POLE_ID IS NOT NULL
                GROUP BY POLE_ID, SENSOR_ID, CONCAT(LEFT(TIME_ID, 10), 'T00:00:00')
                ORDER BY CONVERT(%s, DECIMAL(8,3)) DESC;
...
# SELECT
def selectUv(cur, variable):
    query = """SELECT POLE_ID, SENSOR_ID, (SELECT PART_NAME FROM TB_IOT_POLE_INFO_DUMMY WHERE SENSOR_ID = S1.SENSOR_ID) AS PART_NAME
                , CONCAT(LEFT(TIME_ID, 10), 'T00:00:00') AS TIME_START
                , CONCAT(LEFT(TIME_ID, 10), 'T23:59:59') AS TIME_END
                FROM TB_IOT_POLE_SECOND S1
                WHERE %s IS NOT NULL
                AND CONVERT(%s, DECIMAL(8,3)) > 0
                AND CONVERT(%s, DECIMAL(8,3)) != 200
                AND POLE_ID != ''
                AND POLE_ID IS NOT NULL
                GROUP BY POLE_ID, SENSOR_ID, CONCAT(LEFT(TIME_ID, 10), 'T00:00:00')
                ORDER BY CONVERT(%s, DECIMAL(8,3)) DESC;
...
def selectPress(cur, variable):
    query = """SELECT POLE_ID, SENSOR_ID, (SELECT PART_NAME FROM TB_IOT_POLE_INFO_DUMMY WHERE SENSOR_ID = S1.SENSOR_ID) AS PART_NAME
                ...
                WHERE %s IS NOT NULL
                AND CONVERT(%s, DECIMAL(8,3)) > 0
                AND CONVERT(%s, DECIMAL(8,3)) != 100
                AND CONVERT(%s, DECIMAL(8,3)) NOT BETWEEN 973 AND 1027
                AND POLE_ID != ''
                AND POLE_ID IS NOT NULL
                GROUP BY POLE_ID, SENSOR_ID, CONCAT(LEFT(TIME_ID, 10), 'T00:00:00')
                ORDER BY CONVERT(%s, DECIMAL(8,3)) DESC;
...
# SELECT
def selectUvc(cur, variable):
    query = """SELECT POLE_ID, SENSOR_ID, (SELECT PART_NAME FROM TB_IOT_POLE_INFO_DUMMY WHERE SENSOR_ID = S1.SENSOR_ID) AS PART_NAME
                , CONCAT(LEFT(TIME_ID, 10), 'T00:00:00') AS TIME_START
                , CONCAT(LEFT(TIME_ID, 10), 'T23:59:59') AS TIME_END
                FROM TB_IOT_POLE_SECOND S1
                WHERE %s IS NOT NULL
                AND CONVERT(%s, DECIMAL(8,3)) > 4.000
                AND POLE_ID != ''
                AND POLE_ID IS NOT NULL
                GROUP BY POLE_ID, SENSOR_ID, CONCAT(LEFT(TIME_ID, 10), 'T00:00:00')
                ORDER BY CONVERT(%s, DECIMAL(8,3)) DESC
...
def getSensorData(dir_data, pole_id, sensor_id, part_name, time_start, time_end, resample_how):
    ...
    df = pd.read_csv('{}{}.csv'.format(dir_data, pole_id), encoding='euc-kr', low_memory=False)
    df = df[variable_all]
...
def main(sel_pole_id, pole_top, variable, path_dir, period, period_image, time_start, time_end, resample_how):
    ...
    if variable == 'AMBIENT':
        df_pole_list = selectAmbient(cur, variable)
    elif variable == 'UV':
        df_pole_list = selectUv(cur, variable)
    elif variable == 'PRESS':
        df_pole_list = selectPress(cur, variable)
    elif variable == 'USN':
        df_pole_list = selectUsn(cur, variable)
    elif variable == 'UVC':
        df_pole_list = selectUvc(cur, variable)
    ...
    for idx in df_pole_list.index.values:
        ...
        time_start = df_pole_list['TIME_START'][idx]
        time_end = df_pole_list['TIME_END'][idx]
...
if __name__ == '__main__':
    ...
    if period == '1':
        ...
        time_start = '2017-06-20 00:00:00'
