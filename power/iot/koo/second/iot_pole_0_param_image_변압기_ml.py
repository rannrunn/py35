...
limit_data = {'TEMP':[-18.5, 60], 'HUMI':[0, 100], 'PITCH':[-180, 180], 'ROLL':[-90, 90], 'AMBIENT':[0, 81900], 'UV':[0, 20.48], 'PRESS':[0, 1100], 'BATTERY':[0, 100], 'SHOCK':[0, 1], 'USN':[0, 3000], 'NTC':[-20, 120], 'UVC':[0, 5]}
limit_ylim = {'TEMP':[-18.5, 60], 'HUMI':[-5, 105], 'PITCH':[-185, 185], 'ROLL':[-95, 95], 'AMBIENT':[-100, 32000], 'UV':[-2, 23], 'PRESS':[-10, 1110], 'BATTERY':[-5, 105], 'SHOCK':[-1, 2], 'USN':[-10, 290], 'NTC':[-25, 125], 'UVC':[-1, 6]}
...
def process(variable, dir_data, dir_output, period_image, pole_id, time_start, time_end, resample_how, cnt_data):
    ...
    # df_pole = normalization(df_pole)
    # df_pole = df_pole.rolling(5).mean()
    ...
    # 일별 데이터
        ...
        while range_start < index_end:
            ...
            if len(df_data[df_data.iloc[:, 1].notnull()]) == step_day:
                result = saveImage(df_data, dir_output, period_image, pole_id, cnt_data)
                if result == False:
                    break
...
def getTransformerData(dir_data, pole_id, time_start, time_end, resample_how):
    ...
    for idx in df_sensor.index.values:
        ...
        df_temp.columns = ['TIME_ID', sensor_id + '_' + part_name]
...
def saveImage(df, dir_output, period_image, pole_id, cnt_data):
    ...
    try:
        ...
        ibuff = input('라벨을 선택하시오:')
        print(type(ibuff))
        print(ibuff)
        if ibuff == '2':
            with open("./x_train.csv", "w") as f:
                writer = csv.writer(f)
                writer.writerows(list_x_train)
            with open("./y_train.csv", "w") as f:
                writer = csv.writer(f)
                writer.writerows(list_y_train)
            return False

        list_arr = []
        list_arr.append(np.zeros(24))
        list_arr.append(np.zeros(24))
        list_arr.append(np.zeros(24))
        arr_0 = np.array([])
        arr_1 = np.array([])
        ...
        for item in df.columns.values:
            list_arr[cnt] = np.array(df[item].values)
            cnt += 1

        arr_0 = np.append(arr_0, list_arr[0])
        arr_0 = np.append(arr_0, list_arr[1])
        arr_0 = np.append(arr_0, list_arr[2])

        arr_1 = np.append(arr_1, list_arr[0])
        arr_1 = np.append(arr_1, list_arr[2])
        arr_1 = np.append(arr_1, list_arr[1])

        arr_2 = np.append(arr_2, list_arr[1])
        arr_2 = np.append(arr_2, list_arr[2])
        arr_2 = np.append(arr_2, list_arr[0])

        arr_3 = np.append(arr_3, list_arr[1])
        arr_3 = np.append(arr_3, list_arr[0])
        arr_3 = np.append(arr_3, list_arr[2])

        arr_4 = np.append(arr_4, list_arr[2])
        arr_4 = np.append(arr_4, list_arr[0])
        arr_4 = np.append(arr_4, list_arr[1])

        arr_5 = np.append(arr_5, list_arr[2])
        arr_5 = np.append(arr_5, list_arr[1])
        arr_5 = np.append(arr_5, list_arr[0])

        list_x_train.append(list(arr_0))
        list_x_train.append(list(arr_1))
        list_x_train.append(list(arr_2))
        list_x_train.append(list(arr_3))
        list_x_train.append(list(arr_4))
        list_x_train.append(list(arr_5))

        list_y_train.append(ibuff)
        list_y_train.append(ibuff)
        list_y_train.append(ibuff)
        list_y_train.append(ibuff)
        list_y_train.append(ibuff)
        ...
        print(list_y_train)
        print(np.shape(list_y_train))

        print('CNT:{}, FILE_NAME:{}, LABEL:{}'.format(cnt_data, file_name, ibuff))
...
def main(sel_pole_id, pole_top, variable, path_dir, period, period_image, time_start, time_end, resample_how):
    ...
    cnt = 0
    list_pole = []
    for item in df_pole_list['POLE_ID'].unique():
        cnt += 1
