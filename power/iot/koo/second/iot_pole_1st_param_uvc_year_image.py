...
variable_all = ['TIME_ID', 'POLE_ID', 'SENSOR_ID', 'PART_NAME', 'TEMP', 'HUMI', 'PITCH', 'ROLL','AMBIENT','PRESS', 'SHOCK', 'NTC', 'UVC', 'UV']
variable_plot = ['TEMP', 'HUMI', 'PITCH', 'ROLL','AMBIENT','PRESS', 'SHOCK', 'NTC', 'UVC', 'UV']
limit_data = {'AMBIENT':[0, 81900],'BATTERY':[0, 100],'HUMI':[0, 100],'TEMP':[-15, 65],'PITCH':[-180, 180],'ROLL':[-90, 90],'UV':[0, 20.48],'PRESS':[0, 1100],'SHOCK':[0, 1],'NTC':[-20, 120],'UVC':[0, 5]}
limit_ylim = {'AMBIENT':[-100, 82000], 'BATTERY':[-5, 105], 'HUMI':[-5, 105], 'TEMP':[-15, 65], 'PITCH':[-185, 185], 'ROLL':[-95, 95], 'UV':[-1, 22], 'PRESS':[-10, 1110],'SHOCK':[-1, 2],'NTC':[-22, 122],'UVC':[-1, 6]}
...
def main(path_dir, order, resample_how):
    ...
    df_pole_ntc = pd.read_csv('{}iot_pole_1st_param_uvc.csv'.format(path_dir), encoding = "euc-kr")
    df_pole_ntc = df_pole_ntc[['POLE_ID', 'SENSOR_ID', 'PART_NAME']].drop_duplicates()

    print(len(df_pole_ntc['SENSOR_ID'].drop_duplicates()))
    print(len(df_pole_ntc['POLE_ID'].drop_duplicates()))
    ...
    cnt = 0
    list_pole = []
    for idx in df_pole_ntc.index.values:
        cnt += 1
        print(cnt)
        pole_id = df_pole_ntc['POLE_ID'][idx]
        sensor_id = df_pole_ntc['SENSOR_ID'][idx]
        part_name = df_pole_ntc['PART_NAME'][idx]
