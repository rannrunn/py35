...
for i in range(len(chart)):
    ...
    df.columns = ["Timestamp", col_names[0], col_names[1], col, col + '_pre']
    ...
    df_data_left.append(df_data_tmp[loc_start: loc_end])

    # 실제값
    df_data_tmp = df.iloc[:, 2].values
    df_data_tmp = df_data_tmp.astype(float)
    df_data_right.append(df_data_tmp[loc_start: loc_end])

    # 실제값
    df_data_tmp = df.iloc[:, 3].values
    df_data_tmp = df_data_tmp.astype(float)
    df_data_ori.append(df_data_tmp[loc_start: loc_end])

    # 예상값
    df_data_tmp = df.iloc[:, 4].values
    df_data_tmp = df_data_tmp.astype(float)
    ...
    for j in range(len(df_data_ori[i])):
        ...
        if abs(diff) > 0.20 and df_data_ori[i][j] != 0:
            print( 'sequence:', str(j).rjust(6), 'x:', df_data_time[j], col_names[0] + ':', str(df_data_left[i][j]).ljust(12), col_names[1] + ':', str(df_data_right[i][j]).ljust(12), 'ori:', str(df_data_ori[i][j]).ljust(12), 'pre:', str(df_data_predict[i][j]).ljust(12), 'diff:', str(diff).ljust(12))
            depres_cnt += 1
