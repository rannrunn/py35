...
class DQN:
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            ...
            model = tf.layers.conv2d(model, 62, [2, 2], padding='same', activation=tf.nn.relu)
    ...
    def init_state(self, state):
        ...
        # print('state.shape:', np.shape(self.state)) # state.shape: (6, 10, 4)

    def remember(self, state, action, reward, terminal):
        next_state = np.reshape(state, (self.width, self.height, 1))
        # print('next_state.shape:', np.shape(self.state)) # next_state.shape: (6, 10, 4)
