...
cl_name = '170821_analysis_square_hongtong_lotte_2'
...
y_data = df_ext["y_data"].values ** 2

print(y_data)


W1 = tf.Variable(tf.random_uniform([1], 10000000, 10000000), name='weight_01', dtype=tf.float32)
W2 = tf.Variable(tf.random_uniform([1], 10000000, 10000000), name='weight_02', dtype=tf.float32)
g = 9.80665
...
hypothesis = X1 * W1 * 2 * g + X2 * W2 * 2 * g + b
...
with tf.Session() as sess:
    ...
    try:
        ...
        y_data_sqrt = np.sqrt(y_data)
        predict_data_sqrt = np.sqrt(predict_data)
        difference_data = y_data_sqrt - predict_data_sqrt

        # print(y_data_sqrt)
        print(predict_data_sqrt)
        print(type(y_data_sqrt))
        ...
        print('상관관계', correlation.correlation(y_data_sqrt.tolist(), predict_data_sqrt.tolist()))

        # print('Difference Data:', difference_data)
        print('Max:', np.max(y_data_sqrt - predict_data_sqrt))
